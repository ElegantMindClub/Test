...
if recordData:
    ...
    dlg = gui.DlgFromDict(dictionary=expInfo, sortKeys=False, title='Experiment Info')
    if dlg.OK == False:
        core.quit()
...
def run_random(*funcs):
    function = list(funcs)
    random.shuffle(function)
    tester=[]
    for i in range(len(function)):
        tester.append(function[i-1])
#        tester.append(breaks[i])
    functions=list(tester)
    print(functions)
...
def Covert():
    ...
    def TwoCovert():
        ...
        date = time.strftime("%m_%d")
        expName = 'Face Two Decision Covert'        
        ...
        def instructions():
            genDisplay({'text': 'Press "V" or "B" for the appropriate face',\
                'xPos': 0, 'yPos': centerY+2, 'heightCm': 1, 'color': 'white'}).draw()
...
def Overt():
    def OneOvert():
        ...
        date = time.strftime("%m_%d")
        expName = 'Face One Decision Overt'
        ...
        def instructions():
            genDisplay({'text': 'Press "V" when you see the face',\
                'xPos': 0, 'yPos': centerY+2, 'heightCm': 1, 'color': 'white'}).draw()
            genDisplay({'text': 'Please feel free to move your eye to the stimulus when it appears',\
                'xPos': 0, 'yPos': centerY,'heightCm': 1, 'color': 'white'}).draw()
            genDisplay({'text': 'Press the spacebar to continue',\
                'xPos': 0, 'yPos': centerY-2,'heightCm': 1, 'color': 'white'}).draw()
            win.flip()
    ...
    def TwoOvert():
        ...
        date = time.strftime("%m_%d")
        expName = 'Face Two Decision Overt'
    ...
    def ThreeOvert():
        ...
        date = time.strftime("%m_%d")
        expName = 'Face Three Decision Overt'

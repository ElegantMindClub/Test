...
def interpretPair(pair):
    ...
    yangle = yangles[round((pair-int(pair))/0.1)]
    letter = random.choice(letters)
    return {'angle': angle, 'dir': direction, 'letter': letter, 'yangle': yangle}
...
for pair in pairs:
    ...
    ydisplacement = displaceCalc(trialInfo['yangle'])
    yPos = centerY + ydisplacement
    
    stimheight = displaceCalc(4)*heightMult
    displayInfo = {'text': trialInfo['letter'], 'xPos': xPos, 'yPos': yPos, 'heightCm': stimheight, 'color': 'white'}
    ...
    if buffer > 0:
        if checkcorrect(key[0], trialInfo['letter']):
            output = (trialInfo['letter'], trialInfo['dir'], trialInfo['angle'], reactionTime, trialInfo['yangle'])
            csvOutput(output, fileName)
        else:
            mistakedict[mistakes] = trialInfo
            mistakes += 1
            output = (trialInfo['letter'], trialInfo['dir'], trialInfo['angle'], 0, trialInfo['yangle'])
            csvOutput(output, fileName)
    else:
        mistakedict[mistakes] = trialInfo
        mistakes += 1
